...
class SensorConsole(cmd.Cmd):
    ...
    def do_show(self, arg):
        ...
        if parts[0] in ["feature", "feat", "f"]:
            if self.selected_sensor:
                if parts[1] and parts[2]:
                    steps = int(parts[2])
                    data_feat = self.selected_sensor.get_feature(parts[1]).get_rounded_copy(weight=WeightingFunctions.DELTA,step=steps)
                    print("done fft")
                    data_raw = [self.selected_sensor.get_feature("Raw")[i] for i in range(0,len(self.selected_sensor.get_feature("Raw")), min(steps, 500))]
                    print("done raw")
                    fig, raw, fft = show_Feature(raw_data=data_raw, data=data_feat, name=parts[0])
                    plt.show()
                else:
                    pass
            else:
                print("No Sensor selected")
        print("Format: show <feat/map> <feature> <steps>")
    ...
    def do_import(self, arg):
        ...
        if not os.path.exists(arg):
            print(f".npy File {arg} does not exist!")
            return

        self.device = Importer.import_device_from_npy(arg)
        print(f"{len(self.device.sensors)} sensors loaded!")
        print("You can now use commands like 'introduce', 'save', or 'select <sensor_id>'.")
    ...
    def do_updateFFT_all(self, arg):
        for sensorid in self.device.sensors:
            sensor = self.device.sensors[sensorid]

            sensor.update_fft()
        print("Updated all FFTs")

    def do_updateFFT(self, arg):
        if not self.selected_sensor:
            print("No sensor selected. Use 'select <sensor_id>' first.")
            return
        self.selected_sensor.update_fft()
        print("updated FFT")
    ...
    def do_add_stft_all(self, arg):
        """
        Add a new STFT to all sensors.
        Syntax: add_stft_all <stft_id> <window_size> <window_type> <hop_size>
        Example: add_stft_all stft_1 512 hann 256

        For each sensor in the device, this command adds a new STFT with the given parameters.
        If the sensor has the "Raw" feature, the STFT is automatically computed.
        """
        if not self.device:
            print("No device loaded. Use 'load <directory>' or 'import <file>' first.")
            return

        parts = arg.split()
        if len(parts) != 4:
            print("Usage: add_stft_all <stft_id> <window_size> <window_type> <hop_size>")
            return
        ...
        for sensor_id, sensor in self.device.sensors.items():
            try:
                sensor.add_stft(stft_id=stft_id, window_size=window_size, window_type=window_type, hop_size=hop_size)
                new_stft = sensor.stfts[-1]
                raw_feature = sensor.get_feature("Raw")
                if raw_feature is not None:
                    new_stft.compute_stft(raw=raw_feature)
                    print(
                        f"Sensor {sensor_id}: STFT {new_stft.id} added and computed with window_size={window_size}, window_type='{window_type}', hop_size={hop_size}.")
                else:
                    print(
                        f"Sensor {sensor_id}: STFT {new_stft.id} added, but RAW feature not found. STFT not computed.")
            except Exception as e:
                print(f"Error adding STFT to sensor {sensor_id}: {e}")
