...
class Sensor(Saveable):
    def __init__(self, hdf5: h5py.Group  = None, features: dict[Feature] = None, file: str = None, sensorID: str = "",
    ...
    def set_raw(self, raw : list[np.complex128]):
        if self.get_feature("Raw"):
            self.get_feature("Raw").values = raw
        else:
            self.features["Raw"] = Feature(name="Raw",values=raw)

    def update_fft(self):
        # Hole das "Raw"-Feature, das das Eingangssignal enthalten sollte.
        raw_feature = self.get_feature("Raw")
        if raw_feature is None:
            raise ValueError("Raw feature not found or has no data; cannot compute FFT.")

        # Berechne die FFT des Rohsignals
        fft_values = np.fft.fft(raw_feature)

        # Überprüfe, ob bereits ein "FFT"-Feature existiert und aktualisiere es, ansonsten füge es hinzu.
        fft_feature = self.get_feature("FFT")
        if fft_feature is None:
            self.features["FFT"] = Feature(name="FFT", values=fft_values)
        else:
            fft_feature = fft_values
    ...
    def save(self, destination: h5py.Group) -> None:
        ...
        if self.sensortype is Sensortype:
            sensorgroup.attrs["sensortype"] = self.sensortype.name
        else:
            sensorgroup.attrs["sensortype"] =  str(self.sensortype)
        ...
        for featurekey in self.features.keys():
            ...
            dataset.attrs["peak"] = "" if feature.peak is not None else [-1, -1] #TODO
